...
class NN:
    def __init__(self, input_size: int, hidden_size: int, output_size: int,
        ...
        self.linear1 = Linear(input_size, hidden_size, weight_init_fn,
                              learning_rate)
        self.sigmoid = Sigmoid()
        self.linear2 = Linear(hidden_size, output_size, weight_init_fn,
                              learning_rate)
    ...
    def forward(self, x: np.ndarray, y: int) -> Tuple[np.ndarray, float]:
        ...
        return self.softmax_ce.forward(z2, y)
    ...
    def backward(self, y: int, y_hat: np.ndarray) -> None:
        ...
        :param y_hat: prediction with shape (num_classes,)
        """
        dz2 = self.softmax_ce.backward(y, y_hat)
        da1 = self.linear2.backward(dz2)
        dz1 = self.sigmoid.backward(da1)
    ...
    def train(self, X_tr: np.ndarray, y_tr: np.ndarray,
        ...
        for _ in range(n_epochs):
            X_tr_shuff, y_tr_shuff = shuffle(X_tr, y_tr, _)
            for x_i, y_i in zip(X_tr_shuff, y_tr_shuff):
                y_hat, _ = self.forward(x_i, y_i)
                self.backward(y_i, y_hat)
                self.step()
